          ...
          <tbody>
            <?php foreach ($penjualan->barangs as $i => $barang) : ?>
              <tr>
                <td><?= $i + 1; ?></td>
                <td><?= $barang->nama; ?></td>
                <td><?= rupiah($barang->pivot->harga); ?></td>
                <td><?= $barang->pivot->jumlah; ?></td>
                <td><?= rupiah($barang->pivot->harga * $barang->pivot->jumlah); ?></td>
              </tr>
            <?php endforeach; ?>
          </tbody>
        </table>
      ...
      <div class="card card-body p-2 p-md-3">
        ...
        <div class="border-top table-responsive">
          ...
          <table id="datatable-barang" class="table table-striped table-bordered mytable" style="width:100%">
            ...
            <tbody>
              <?php foreach ($transaksies as $i => $transaksi) : ?>
                <tr>
            ...
            <tfoot>
              ...
              <th class="text-danger font-weight-bold">
                <div class="d-flex justify-content-between">
                  <span>Rp.</span>
                  <span><?= rupiah(array_sum(array_column($transaksies, "jumlah"))); ?></span>
                </div>
              </th>
              <th class="text-danger font-weight-bold">
                <div class="d-flex justify-content-between">
  ...
  <div class="col-md-5 order-md-1 px-0">
    <div class="card card-body p-2 p-md-3">
      <div class="row">
        ...
        <div class="col-6 col-lg-4 px-0 px-md-2">
          <div class="form-group pt-md-2 pb-0 pt-1">
            <label class="mb-0 mb-sm-2" for="#no-invoice">No Invoice</label>
            <input class="form-control p-1" readonly value="<?= $penjualan->no_invoice; ?>" id="no-invoice">
          </div>
        </div>
        <div class="col-6 col-lg-4 px-0 px-md-2">
          <div class="form-group pt-md-2 pb-0 pt-1">
            <label class="mb-0 mb-sm-2" for="created_at">Tanggal Beli</label>
            <input class="form-control p-1" readonly value="<?= date("F, m Y", strtotime($penjualan->created_at)); ?>" id="created_at">
          </div>
        </div>
        <div class="col-6 col-lg-4 px-0 px-md-2">
          <div class="form-group pt-md-2 pb-0 pt-1">
            <label class="mb-0 mb-sm-2" for="jatuh_tempo"><strong class="text-danger">Jatuh Tempo</strong></label>
            <input class="form-control p-1 text-danger font-weight-bold" readonly value="<?= date("m, F Y", strtotime($penjualan->jatuh_tempo)); ?>" id="jatuh_tempo">
          </div>
        </div>
        <div class="col-6 col-lg-4 px-0 px-md-2">
          <div class="form-group pt-md-2 pb-0 pt-1">
            <label class="mb-0 mb-sm-2"><strong class="text-danger">Total Transaksi</strong></label>
            <input class="form-control p-1 text-danger font-weight-bold" readonly value="<?= rupiah($penjualan->jumlah_bayar); ?>">
          </div>
        </div>
        <div class="col-6 col-lg-4 px-0 px-md-2">
          <div class="form-group pt-md-2 pb-0 pt-1">
        ...
        <form action="" method="post" id="form-bayar">
          ...
          <div class=" d-flex flex-column flex-md-row justify-content-md-end">
            <div class="col-md-7 pr-md-2 px-0">
              <input type="hidden" name="jumlah_pinjaman" value="<?= $penjualan->jumlah_bayar - $penjualan->dibayar; ?>">
  ...
  $(document).ready(function bayar() {
    ...
    $("#form-bayar").on("submit", function(e) {
      ...
      if (AutoNumeric.getNumber("#jumlah") > 1)
        $.ajax({
          url: baseUrl + `admin/transaksi/piutang/penjualan`,
          type: "POST",
          data: $(this).serialize(),
