...
<script>
  ...
  $(document).ready(() => {
    $("#datatable-keranjang_pembelian").on("click", ".btn-update", function(e) {
      e.preventDefault();
      let keranjang = datatable_keranjang.row($(this).parents('tr')).data();
      $.ajax({
        url: baseUrl + "admin/barang/update/from_pembelian/" + keranjang.barang_id,
        type: "PUT",
        data: {
          harga_beli: AutoNumeric.getNumber("[name=harga_beli]"),
